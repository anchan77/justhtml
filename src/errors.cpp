/// @file errors.cpp
/// @brief Implementation of error message generation.
...
// Messages that don't need tag_name interpolation.
const std::unordered_map<std::string, std::string> kStaticMessages = {
    // TOKENIZER ERRORS
    // DOCTYPE errors
    ...
    {"eof-in-doctype-public-identifier",
     "Unexpected end of file in DOCTYPE public identifier"},
    {"eof-in-doctype-system-identifier",
     "Unexpected end of file in DOCTYPE system identifier"},
    ...
    {"missing-whitespace-between-doctype-public-and-system-identifiers",
     "Missing whitespace between DOCTYPE identifiers"},
    ...
    {"unexpected-character-after-doctype-public-keyword",
     "Unexpected character after PUBLIC keyword"},
    {"unexpected-character-after-doctype-system-keyword",
     "Unexpected character after SYSTEM keyword"},
    {"unexpected-character-after-doctype-public-identifier",
     "Unexpected character after public identifier"},
    ...
    {"eof-in-script-html-comment-like-text",
     "Unexpected end of file in script with HTML-like comment"},
    ...
    {"illegal-codepoint-for-numeric-entity",
     "Invalid codepoint in numeric character reference"},
    ...
    {"unexpected-doctype-in-foreign-content",
     "Unexpected DOCTYPE in SVG/MathML content"},
    ...
    {"invalid-codepoint-in-foreign-content",
     "Invalid character in SVG/MathML content"},
    ...
    {"unexpected-hidden-input-in-table",
     "Hidden input in table triggers foster parenting"},
    ...
    {"unexpected-token-after-after-frameset",
     "Unexpected content after frameset closed"},
    ...
    {"unexpected-characters-in-template-column-group",
     "Text not allowed in template column group"},
    // Foreign content
    {"unexpected-html-element-in-foreign-content",
     "HTML element breaks out of SVG/MathML content"},
    // Miscellaneous
    {"adoption-agency-1.3", "Misnested tags require adoption agency algorithm"},
};

// Messages that require tag_name interpolation.
// Pattern: the tag_name is inserted at the placeholder position.
struct TagMessage {
    std::string prefix;
    std::string suffix;
    bool is_end_tag;  // true = </tag>, false = <tag>
};

const std::unordered_map<std::string, TagMessage> kTagMessages = {
    {"expected-doctype-but-got-start-tag", {"Expected DOCTYPE but got <", "> tag", false}},
    {"expected-doctype-but-got-end-tag", {"Expected DOCTYPE but got </", "> tag", true}},
    {"unexpected-start-tag", {"Unexpected <", "> start tag", false}},
    {"unexpected-end-tag", {"Unexpected </", "> end tag", true}},
    {"unexpected-end-tag-before-html", {"Unexpected </", "> end tag before <html>", true}},
    {"unexpected-end-tag-before-head", {"Unexpected </", "> end tag before <head>", true}},
    {"unexpected-end-tag-after-head", {"Unexpected </", "> end tag after <head>", true}},
    {"unexpected-start-tag-ignored",
     {"<", "> start tag ignored in current context", false}},
    {"unexpected-start-tag-implies-end-tag",
     {"<", "> start tag implicitly closes previous element", false}},
    {"expected-closing-tag-but-got-eof",
     {"Expected </", "> closing tag but reached end of file", true}},
    {"expected-named-closing-tag-but-got-eof",
     {"Expected </", "> closing tag but reached end of file", true}},
    {"unexpected-start-tag-implies-table-voodoo",
     {"<", "> start tag in table triggers foster parenting", false}},
    {"unexpected-end-tag-implies-table-voodoo",
     {"</", "> end tag in table triggers foster parenting", true}},
    {"unexpected-start-tag-in-column-group",
     {"<", "> start tag not allowed in <colgroup>", false}},
    {"unexpected-start-tag-in-template-column-group",
     {"<", "> start tag not allowed in template column group", false}},
    {"unexpected-start-tag-in-template-table-context",
     {"<", "> start tag not allowed in template table context", false}},
    {"unexpected-start-tag-in-cell-fragment",
     {"<", "> start tag not allowed in cell fragment context", false}},
    {"unexpected-end-tag-in-foreign-content",
     {"Mismatched </", "> end tag in SVG/MathML content", true}},
    {"unexpected-end-tag-in-fragment-context",
     {"</", "> end tag not allowed in fragment parsing context", true}},
    {"end-tag-too-early",
     {"</", "> end tag closed early (unclosed children)", true}},
    {"non-void-html-element-start-tag-with-trailing-solidus",
     {"<", "/> self-closing syntax on non-void element", false}},
    {"image-start-tag", {"Deprecated <", "> tag (use <img> instead)", false}},
};
...
std::string generate_error_message(const std::string& code,
    ...
    auto it = kStaticMessages.find(code);
    if (it != kStaticMessages.end()) {
        return it->second;
    }

    // Check tag-interpolated messages
    auto tit = kTagMessages.find(code);
    if (tit != kTagMessages.end()) {
        const auto& msg = tit->second;
        std::string name = tag_name.value_or("unknown");
        return msg.prefix + name + msg.suffix;
    }
